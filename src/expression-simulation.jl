...
function fit_expression_model_cgan(
    ...
    gen_opt = ADAM(1e-3, (0.3, 0.9))
    dis_opt = ADAM(1e-3, (0.3, 0.9))
    ...
    gen_cpu = gen |> cpu
    dis_cpu = dis |> cpu
    BSON.@save output_params_filename xmean xstd noise_dim gen_cpu dis_cpu
end
...
function sample_expression_model_cgan(
    ...
    x̄ = (cpu(gen(labels, noise)) .* xstd) .+ xmean
